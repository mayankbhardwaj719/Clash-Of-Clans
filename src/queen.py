...
class Queen():
    def __init__(self,row,column):
        ...
        self.speed = 1
        self.attack_radius = 5 # radius in which the queen attacks around the 8X8 Area
        self.specialattack_radius = 16 # radius in which the queen attacks around the 9X9 Area
        self.lastmove = " "
    ...
    def move(self,ch,vil):
        if(self.health > 0):
            flag = False # Flag to indicate if the queen can move at the desired position or not
            ...
            if(flag):
                self.lastmove = ch
                vil.is_present[self.row][self.column] = 7
    ...
    def attack(self,ch,vil):
        if(self.health>0):
            ...
            if(self.lastmove=='w'):
                temp_row = max(0,self.row-8)
                temp_col = self.column
            elif(self.lastmove=='a'):
                temp_row = self.row
                temp_col = max(0,self.column-8)
            elif(self.lastmove=='s'):
                temp_row = min(vil.rows,self.row+8)
                temp_col = self.column
            elif(self.lastmove=='d'):
                temp_row = self.row
                temp_col = min(vil.columns,self.column+8)
            ...
            for temp in (vil.huts):
                x = temp.row
                y = temp.column
                if( math.sqrt((x-temp_row)**2 + (y-temp_col)**2) <= self.attack_radius ):
            ...
            if(len(vil.townhall)>0):
                ...
                for i in range(4):
                    flag = 0
                    for j in range(3):
                        if( math.sqrt((temp_th.topleft-temp_row+i)**2 + (temp_th.topright-temp_col+j)**2) <= self.attack_radius ):
    ...
    def specialAttack(self,ch,vil):
        if(self.health>0):
            ...
            if(self.lastmove=='w'):
                temp_row = max(0,self.row-16)
                temp_col = self.column
            elif(self.lastmove=='a'):
                temp_row = self.row
                temp_col = max(0,self.column-16)
            elif(self.lastmove=='s'):
                temp_row = min(vil.rows,self.row+16)
                temp_col = self.column
            elif(self.lastmove=='d'):
                temp_row = self.row
                temp_col = min(vil.columns,self.column+16)
            ...
            for temp in (vil.huts):
                x = temp.row
                y = temp.column
                if( math.sqrt((x-temp_row)**2 + (y-temp_col)**2) <= self.specialattack_radius ):
                    temp.hitpoint -= self.damage
                    # vil.is_present[temp.row][temp.column] = 3
                    if(temp.isDestroyed()):
                        vil.huts.remove(temp)
